...
export async function syncDailyData(): Promise<SyncLog> {
  ...
  try {
    ...
    feishuData.forEach((record, index) => {
      // 从飞书表格截图看到，应该使用"每日利润汇总"字段
      const profitValue = getFieldValue(record, '每日利润汇总');
      ...
      // 按照表格中实际的日期顺序来映射：9/24, 9/23, 9/22...
      const currentDate = new Date('2025-09-24'); // 基准日期（2025年）
      currentDate.setDate(currentDate.getDate() - index); // 往前推算
      
      const dailyProfit: DailyProfit = {
        date: currentDate.toISOString().split('T')[0],
        daily_profit: getFieldValue(record, '每日盈利'), // 每日盈利字段
        profit_summary: profitValue, // 每日利润汇总字段
      ...
      console.log(`[Sync] 处理每日数据: ${dailyProfit.date} = 利润¥${dailyProfit.daily_profit}, 货款¥${dailyProfit.payment_expense} (索引${index})`);
    });
...
// 完整同步函数
export async function syncAllData(): Promise<{ daily: SyncLog; monthly: SyncLog; yearly: SyncLog }> {
